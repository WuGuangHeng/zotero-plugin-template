...
export class RAGFlowService {
  ...
  public static async createDataset(name: string): Promise<string> {
    try {
      ...
      Zotero.debug(
        `[RAGFlow] createDataset: API密钥长度: ${this.apiKey ? this.apiKey.length : 0}`,
      );
      ...
      Zotero.debug(
        `[RAGFlow] createDataset: 请求体: ${JSON.stringify(requestBody)}`,
      );
      ...
      if (!response.ok) {
        ...
        Zotero.debug(
          `[RAGFlow] createDataset: HTTP错误: ${response.status} ${response.statusText}`,
        );
        Zotero.debug(`[RAGFlow] createDataset: 错误响应内容: ${errorText}`);
        throw new Error(
          `创建数据集HTTP错误: ${response.status} ${response.statusText}`,
        );
      }
      ...
      try {
        result = JSON.parse(
          responseText,
        ) as RAGFlowAPIResponse<DatasetResponse>;
      } catch (parseError) {
        Zotero.debug(
          `[RAGFlow] createDataset: JSON解析错误: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `JSON解析错误: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
      }

      Zotero.debug(
        `[RAGFlow] createDataset: 响应码: ${result.code}, 消息: ${result.message || "无"}`,
      );
      ...
      Zotero.debug(
        `[RAGFlow] createDataset: 数据集创建成功，ID: ${result.data.id}`,
      );
  ...
  public static async uploadFiles(
    ...
    try {
      ...
      Zotero.debug(
        `[RAGFlow] uploadFiles: 文件列表: ${files.map((f) => f.name).join(", ")}`,
      );
      ...
      const supportedFiles = files.filter((file) => {
        ...
        if (isHTML || isSnapshot) {
          Zotero.debug(
            `[RAGFlow] uploadFiles: 跳过不支持的HTML快照文件: ${file.name}`,
          );
          return false;
        }
      ...
      Zotero.debug(
        `[RAGFlow] uploadFiles: 过滤后剩余 ${supportedFiles.length} 个支持的文件`,
      );
      ...
      for (const file of supportedFiles) {
        Zotero.debug(
          `[RAGFlow] uploadFiles: 开始处理文件: ${file.name}, 路径: ${file.path}`,
        );
        ...
        try {
          ...
          Zotero.debug(
            `[RAGFlow] uploadFiles: 文件内容读取成功，大小: ${fileContent.length} 字节`,
          );
            ...
            xhr.onload = function () {
              if (xhr.status === 200) {
                try {
                  ...
                  if (response.code === 0) {
                    Zotero.debug(
                      `[RAGFlow] uploadFiles: 文件 ${file.name} 上传成功`,
                    );
                    resolve(response);
                  } else {
                    // 处理API错误，如不支持的文件类型
                    if (
                      ...
                      Zotero.debug(
                        `[RAGFlow] uploadFiles: 文件类型不支持: ${file.name} (${file.mimeType})`,
                      );
          ...
          if (
            ...
            Zotero.debug(
              `[RAGFlow] uploadFiles: 跳过不支持的文件类型: ${file.name}`,
            );
            continue;
          }

          Zotero.debug(
            `[RAGFlow] uploadFiles: 上传文件时出错: ${errorMessage}`,
          );
          if (error instanceof Error && error.stack) {
      ...
      Zotero.debug(
        `[RAGFlow] uploadFiles: 获取到 ${documentIds.length} 个文档ID`,
      );
      ...
      // 5. 返回数据集ID作为知识库ID
      Zotero.debug(
        `[RAGFlow] uploadFiles: 上传过程完成，知识库ID: ${datasetId}`,
      );
      return datasetId;
    } catch (error: unknown) {
      const errorMessage =
  ...
  private static async getDocumentIds(datasetId: string): Promise<string[]> {
    try {
      Zotero.debug(
        `[RAGFlow] getDocumentIds: 开始获取数据集 ${datasetId} 的文档列表`,
      );
      ...
      if (!response.ok) {
        const errorText = await response.text();
        Zotero.debug(
          `[RAGFlow] getDocumentIds: HTTP错误: ${response.status} ${response.statusText}`,
        );
        Zotero.debug(`[RAGFlow] getDocumentIds: 错误响应内容: ${errorText}`);
        throw new Error(
          `获取文档列表HTTP错误: ${response.status} ${response.statusText}`,
        );
      }
      ...
      try {
        result = JSON.parse(
          responseText,
        ) as RAGFlowAPIResponse<DocumentListResponse>;
      } catch (parseError) {
        Zotero.debug(
          `[RAGFlow] getDocumentIds: JSON解析错误: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `JSON解析错误: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
      ...
      // 检查API响应码
      Zotero.debug(
        `[RAGFlow] getDocumentIds: 响应码: ${result.code}, 消息: ${result.message || "无"}`,
      );
      ...
      if (
        !result.data ||
        !result.data.docs ||
        !Array.isArray(result.data.docs)
      ) {
        Zotero.debug(
          `[RAGFlow] getDocumentIds: 响应数据结构不正确: ${JSON.stringify(result.data)}`,
        );
      ...
      Zotero.debug(
        `[RAGFlow] getDocumentIds: 获取到 ${documentIds.length} 个文档ID: ${documentIds.join(", ")}`,
      );
   ...
   * 解析文档（处理文档）
   */
  private static async parseDocuments(
    datasetId: string,
    documentIds: string[],
  ): Promise<boolean> {
    try {
  ...
  public static async checkDatasetStatus(datasetId: string): Promise<{
    ...
    try {
      ...
      const result =
        (await response.json()) as unknown as RAGFlowAPIResponse<DatasetResponse>;
   ...
   * @param params 聊天助手参数
   */
  public static async createChatAssistant(
    datasetId: string,
    name: string,
    ...
    try {
      ...
      try {
        result = JSON.parse(
          responseText,
        ) as RAGFlowAPIResponse<ChatAssistantResponse>;
      } catch (parseError) {
        Logger.error(
          `解析创建聊天助手API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `解析创建聊天助手API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
   ...
   * @param params 聊天助手参数
   */
  public static async updateChatAssistant(
    chatId: string,
    name: string,
    params?: ChatAssistantParams,
  ): Promise<boolean> {
    try {
      ...
      Logger.debug(
        `更新聊天助手请求体: ${JSON.stringify(requestBody, null, 2)}`,
      );
      ...
      try {
        ...
        Logger.error(
          `解析更新聊天助手API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `解析更新聊天助手API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
   ...
   * @param chatId 聊天助手ID
   */
  public static async getChatAssistantDetails(
    chatId: string,
    ...
    try {
      ...
      let result;
      try {
        result = JSON.parse(responseText) as RAGFlowAPIResponse<
          ChatAssistantResponse[]
        >;
      } catch (parseError) {
        Logger.error(
          `解析获取聊天助手详情API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `解析获取聊天助手详情API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
  ...
  public static async createSession(
    ...
    try {
      ...
      try {
        result = JSON.parse(
          responseText,
        ) as RAGFlowAPIResponse<SessionResponse>;
      } catch (parseError) {
        Logger.error(
          `解析创建会话API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `解析创建会话API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
  ...
  public static async askQuestion(
    ...
    try {
      Logger.info(
        `向聊天助手 ${chatId} 的会话 ${sessionId} 发送问题: ${question}`,
      );
      ...
      try {
        ...
        Logger.error(
          `解析聊天API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
        throw new Error(
          `解析聊天API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
        );
   ...
   * 获取知识库列表
   */
  public static async listDatasets(): Promise<
   ...
   * 获取知识库状态
   */
  public static async getKnowledgeBaseStatus(
    datasetId: string,
  ): Promise<string> {
    try {
