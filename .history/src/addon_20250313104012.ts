...
class Addon {
  ...
  public async onStartup() {
    ...
    // 添加调试信息
    Logger.info(`API Key状态: ${Zotero.Prefs.get(`${config.prefsPrefix}.apiKey`, true) ? "已设置" : "未设置"}`);
  ...
  public async uploadCollectionToRAGFlow(collection: Zotero.Collection) {
    try {
      Zotero.debug(`[RAGFlow] 开始上传集合: ${collection.name}, ID: ${collection.id}`);
      ...
      for (const item of items) {
        ...
        if (item.isAttachment()) {
          Zotero.debug(`[RAGFlow] 条目 ${item.id} (${item.getField('title')}) 是附件，跳过`);
          continue;
        }
        const itemAttachments = item.getAttachments();
        Zotero.debug(`[RAGFlow] 条目 ${item.id} (${item.getField('title')}) 有 ${itemAttachments.length} 个附件`);
        ...
        for (const attachmentID of itemAttachments) {
          ...
          if (attachment.isFileAttachment()) {
            ...
            if (path) {
              ...
              const mimeType = attachment.attachmentContentType || this.guessMimeType(path);
              
              Zotero.debug(`[RAGFlow] 添加附件: ${name}, 路径: ${path}, 类型: ${mimeType}`);
      ...
      // 为常见错误提供更友好的信息
      if (errorMessage.includes("不支持的HTML快照文件") || 
          errorMessage.includes("This type of file has not been supported yet")) {
        userMessage = "上传失败: RAGFlow目前不支持HTML快照文件，请使用PDF或文本文件";
      } else if (errorMessage.includes("没有找到RAGFlow支持的文件类型")) {
        userMessage = "上传失败: 没有找到RAGFlow支持的文件类型。目前支持PDF、TXT等文件，不支持HTML快照";
      } else {
  ...
  public async processQuestion(question: string) {
    try {
      ...
      if (!this.chatAssistantId) {
        ...
        Logger.info(`创建聊天助手: ${assistantName}, 知识库ID: ${this.data.kbId}`);
        
        try {
          this.chatAssistantId = await RAGFlowService.createChatAssistant(this.data.kbId, assistantName);
          Logger.info(`聊天助手创建成功，ID: ${this.chatAssistantId}`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      if (!this.sessionId) {
        ...
        Logger.info(`创建会话: ${sessionName}, 聊天助手ID: ${this.chatAssistantId}`);
        
        try {
          this.sessionId = await RAGFlowService.createSession(this.chatAssistantId, sessionName);
          Logger.info(`会话创建成功，ID: ${this.sessionId}`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      Logger.info(`向聊天助手 ${this.chatAssistantId} 的会话 ${this.sessionId} 发送问题: ${question}`);
      
      // 调用修改后的askQuestion方法，传入正确的参数
      const result = await RAGFlowService.askQuestion(this.chatAssistantId, this.sessionId, question);
      Logger.info("成功获取回答");
      ...
      if (errorMessage.includes("聊天助手") || errorMessage.includes("chat assistant")) {
        // 聊天助手相关错误，尝试重置聊天助手ID
        this.chatAssistantId = undefined;
        progressWindow.createLine({ 
