...
class Addon {
  ...
  public async openCollectionSelector() {
    ...
    const apiKey = Zotero.Prefs.get(`${config.prefsPrefix}.apiKey`) as string;
    if (!apiKey) {
      const progressWindow = this.data.ztoolkit.ProgressWindow.new("RAGFlow 提示");
    ...
    if (!collection) {
      const progressWindow = this.data.ztoolkit.ProgressWindow.new("RAGFlow 错误");
  ...
  public async uploadCollectionToRAGFlow(collection) {
    try {
      // 显示进度窗口
      const progressWindow = this.data.ztoolkit.ProgressWindow.new(
        "RAGFlow 上传",
        { closeOnClick: false }
      );
  ...
  private async processQuestion(question: string) {
    try {
      const progressWindow = this.data.ztoolkit.ProgressWindow.new("RAGFlow 问答");
