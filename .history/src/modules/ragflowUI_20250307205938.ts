  ...
  public static createSettingsUI() {
    ...
    const dialog = new ztoolkit.Dialog(3, 1)
      ...
      .setDialogData({
        ...
        // 修改: onUnload 改为正确的 unloadCallback
        unloadCallback: () => {
          if (dialogData._lastButtonId === "save") {
            //保存前记录
            Zotero.debug("保存前API Key:"+ Zotero.Prefs.get(`${config.prefsPrefix}.apiKey`, true));
            // 保存设置
            ...
            //保存后记录
            Zotero.debug("保存后API Key:"+ Zotero.Prefs.get(`${config.prefsPrefix}.apiKey`, true));
