 ...
 */
  public static createChatAssistantSettingsDialog(datasetId: string, callback: (params: ChatAssistantParams) => void, existingChatId?: string): void {
    ...
    if (existingChatId) {
      ...
      // 添加null检查，防止querySelector返回null
      const dialogContent = dialog.window.document.querySelector(".dialog-content");
      if (dialogContent) {
        dialogContent.appendChild(loadingMessage);
      } else {
        // 如果找不到.dialog-content元素，添加到其他可能存在的容器
        Logger.warn("在对话框中找不到.dialog-content元素");
        try {
          // 尝试其他可能的选择器或直接添加到body
          const alternativeContainer = dialog.window.document.querySelector(".dialog-container") || 
                                      dialog.window.document.body;
          alternativeContainer.appendChild(loadingMessage);
        } catch (e) {
          Logger.error("无法添加加载消息到对话框", e);
        }
      }
      // dialog.window.document.querySelector(".dialog-content").appendChild(loadingMessage);
      
      // 请求现有聊天助手的设置
      try {
        RAGFlowService.getChatAssistantDetails(existingChatId).then(assistant => {
          // 移除加载提示（添加非空检查）
          if (loadingMessage.parentNode) {
            loadingMessage.parentNode.removeChild(loadingMessage);
          } else {
            loadingMessage.remove(); // 现代浏览器支持直接调用remove()
          }
          
          // 填充表单
          if (assistant.llm) {
            dialog.window.document.getElementById("model").value = assistant.llm.model_name || defaultSettings.model;
            dialog.window.document.getElementById("temperature").value = assistant.llm.temperature?.toString() || defaultSettings.temperature.toString();
            dialog.window.document.getElementById("top_p").value = assistant.llm.top_p?.toString() || defaultSettings.top_p.toString();
            dialog.window.document.getElementById("max_tokens").value = assistant.llm.max_tokens?.toString() || defaultSettings.max_tokens.toString();
          }
          
          if (assistant.prompt) {
            dialog.window.document.getElementById("similarity_threshold").value = 
              assistant.prompt.similarity_threshold?.toString() || defaultSettings.similarity_threshold.toString();
            dialog.window.document.getElementById("top_n").value = 
              assistant.prompt.top_n?.toString() || defaultSettings.top_n.toString();
          }
          
          // 更新对话框数据
          dialog.dialogData.model = dialog.window.document.getElementById("model").value;
          dialog.dialogData.temperature = dialog.window.document.getElementById("temperature").value;
          dialog.dialogData.top_p = dialog.window.document.getElementById("top_p").value;
          dialog.dialogData.max_tokens = dialog.window.document.getElementById("max_tokens").value;
          dialog.dialogData.similarity_threshold = dialog.window.document.getElementById("similarity_threshold").value;
          dialog.dialogData.top_n = dialog.window.document.getElementById("top_n").value;
