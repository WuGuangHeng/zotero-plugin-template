 ...
 */
  public static async askQuestion(datasetId: string, question: string): Promise<{answer: string, sources: Array<{content: string, document_name: string}>}> {
    try {
      ...
      Logger.debug(`检索API原始响应: ${responseText}...`);
      ...
      Logger.debug(`聊天API原始响应: ${chatResponseText}`);
      
      let chatResult;
      try {
        chatResult = JSON.parse(chatResponseText) as ChatCompletionResponse;
        
        // 记录解析后的响应结构，帮助调试
        Logger.debug(`聊天API响应结构: ${JSON.stringify({
          hasChoices: !!chatResult.choices,
          choicesLength: chatResult.choices?.length,
          hasMessage: chatResult.choices?.[0]?.message != null,
          messageContent: chatResult.choices?.[0]?.message?.content?.substring(0, 50)
        })}`);
        
        // 检查响应结构并提取答案
        if (chatResult.choices && chatResult.choices.length > 0) {
          // 处理标准格式
          if (chatResult.choices[0].message && chatResult.choices[0].message.content) {
          ...
          // 适配可能的替代格式
          else if (chatResult.choices[0].text || chatResult.choices[0].content) {
            const content = chatResult.choices[0].text || chatResult.choices[0].content;
            Logger.info(`获取到替代格式的回答，长度: ${content.length} 字符`);
            return {
              answer: content,
              sources: sources
            };
          ...
          // 格式不匹配
            ...
            Logger.error("无法从响应中提取内容，响应格式不符合预期", chatResult);
            throw new Error("无法从响应中提取内容，响应格式不符合预期");
          }
        } else {
        ...
        Logger.error(`解析聊天API返回内容失败: ${parseError instanceof Error ? parseError.message : String(parseError)}`, parseError);
        Logger.error(`聊天API原始响应内容: ${chatResponseText}`);
