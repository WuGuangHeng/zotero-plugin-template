...
export class RAGFlowUI {
  ...
  public static createSettingsUI() {
    ...
    const dialog = new ztoolkit.Dialog(3, 1)
      ...
      // 设置对话框数据
      .setDialogData({
        apiKey: Zotero.Prefs.get(`${config.prefsPrefix}.apiKey`) as string || "ragflow-ZkZjFlZmIwZjVhNjExZWZhNDNmMDI0Mm",
        apiUrl: Zotero.Prefs.get(`${config.prefsPrefix}.apiUrl`) as string || "http://127.0.0.1",
        // 添加关闭时的回调函数
