 ...
 */
  ...
  public static createChatAssistantSettingsDialog(datasetId: string, callback: (params: ChatAssistantParams) => void, existingChatId?: string): void {
    ...
    dialog.setDialogData({
      // 初始化一个activeSettings对象，用于存储当前实际使用的设置
      activeSettings: { ...defaultSettings },
      
      // 加载回调 - 当对话框元素创建完成后执行
      loadCallback: async () => {
        // 设置表单元素初始值的函数
        const setFormValues = (settings: any) => {
          const elements = {
          ...
          // 更新activeSettings
          dialog.dialogData.activeSettings = { ...settings };
        };
      ...
      unloadCallback: () => {
        ...
        if (dialog.dialogData._lastButtonId === "save") {
          ...
          // 获取当前活跃的设置
          const activeSettings = dialog.dialogData.activeSettings;
