  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // 创建表单数据对象 - 使用类型断言解决TypeScript错误
        const formData = (Components.classes["@mozilla.org/files/formdata;1"] as any)
                           .createInstance((Components.interfaces as any).nsIFormData);
                            
        // 获取文件对象 - 同样使用类型断言
        Zotero.debug(`[RAGFlow] uploadFiles: 创建文件对象: ${file.path}`);
        const nsFile = (Components.classes["@mozilla.org/file/local;1"] as any)
                         .createInstance((Components.interfaces as any).nsILocalFile || (Components.interfaces as any).nsIFile);
